...
def game_init():
	pygame.init()
	pygame.display.set_caption('Bounce back to shape')
	
	#game_icon = pygame.image.load('carIcon.png')
	#pygame.display.set_icon(game_icon)c
...
def game_intro():
	...
	while intro:
		...
		textSurf, textRect = text_object("Bounce back 2 shape!", largeText)
		textRect.center = ((display_width/2) , (display_height/2))
		game_display.blit(textSurf, textRect)
...
def game_loop():
	...
	thing_startx = random.randrange(100, display_width - 250)
	thing_starty = -600
	thing_speed = 2
	...
	while not gameExit:
		...
		display(dodged, 5, 40)
		display(bounces,5, 0, "Bounces: %d")
		display(thing_speed*10 , 5, 60, "Spd: %d px/s")    
		display(score_game, 5, 20, "Final Score: %d")
		...
		if bounces == 100:
			## TO DO: FLASH LEDs in rainbow sparkles for 2 seconds
			bounces = 0 # reset number of bounces to zero
